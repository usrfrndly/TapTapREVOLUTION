    ...
    if 'title' in request.GET:
        ...
        # User submitted the search form sucessfully
            ...
            if searchedSongId:
                ...
                path = TTR.settings.OUTPUT_FOLDER + "/" + searchedSongId['artist'] + " - " + searchedSongId[
                    'title'] + ".mp3"
                common_log("search_results", "Attempting to download track to the path: " + path)
                ...
                if searchedSongId == download_track(searchedSongId, path):
                    ...
                    run_music_analyzers(path)
                    track_model_id = parse_analyzer_output(searchedSongId, path)


                    # Returns search form/ song selection page
                ...
                return render(request, 'game/SearchForm.html', {'queryMessage': queryMessage,
                                                                ...
                                                                'error': error})
    # Return search form if user did not a submit form
    return render(request, 'game/SearchForm.html')
...
def run_music_analyzers(path_to_track):
    ...
    streaming_extractor_output_file_path = os.path.abspath(
        'game/static/game/scripts/streamingextractoroutputfile.yaml')
...
def parse_analyzer_output(searched_song_id, path):
    with open('game/static/game/scripts/streamingextractoroutputfile.yaml', 'r') as f:
        ...
        # searched_song_id['']
        tr = DownloadedTrack(title=track, artist=artist, album=album, length=song_length, bpm=bpm,beats_position=beats_position, bpm_estimates=bpm_estimates,chord_progression=chords_progression, song_file=path)
        tr.save()
        return tr.id
...
def playttr(request):
    #TODO:Come back here!
       ...
       # track_model_id = request.POST['downloadedId']
    track_model_id = 3
